...
function saveOriginalFlow(modeler) {
  ...
  console.log('💾 Guardando flujo original...');
  // DON'T clear the map if it already has data - only clear on first save
  if (originalFlowMap.size === 0) {
    console.log('💾 Primera vez guardando flujo - inicializando mapa');
  } else {
    console.log('💾 Actualizando flujo existente - conservando datos previos');
  }
  ...
  console.log(`💾 Encontrados ${bpmnTasks.length} elementos BPMN para mapear:`, 
    bpmnTasks.map(t => `${getElementName(t)} (${t.type})`));
  ...
  bpmnTasks.forEach(task => {
    ...
    const nextTasks = outgoingConnections
      ...
      .filter(target => {
        ...
        return validTypes.includes(target.type) && targetName;
      });
    
    if (nextTasks.length > 0) {
      // Store by both ID and name for better reliability
      originalFlowMap.set(taskId, nextTasks);
      originalFlowMap.set(taskName, nextTasks);
      console.log(`💾 Flujo guardado: ${taskName} (${taskId}) -> [${nextTasks.map(t => getElementName(t)).join(', ')}]`);
    }
  });
  
  console.log(`💾 Flujo original guardado completo (${originalFlowMap.size} entradas):`, originalFlowMap);
}
...
function cleanupOrphanedElements(modeler) {
  ...
  elementRegistry.forEach(element => {
    ...
    if (isSpecialElement) {
      ...
      if (incomingConnections.length === 0 && outgoingConnections.length === 0) {
        console.log(`🗑️ Elemento huérfano para limpiar: ${elementName}`);
  ...
  if (elementsToRemove.length > 0) {
    console.log(`🧹 Limpiando ${elementsToRemove.length} elementos huérfanos`);
    try {
